...
Ícone e título na janela do navegador
...
Repare que a tag <link> é um pouco mais complexa que as outras que já usamos. Ela tem dois atributos: rel e href. O atributo rel diz que tipo de conteúdo que queremos referenciar com ela (um ícone, no nosso caso). Já o atributo href diz onde esse conteúdo se encontra. Usaremos novamente essa tag mais para a frente.
...
Na página que estamos desenvolvendo, temos bastante texto escrito. Com tanto texto, talvez o usuário fique meio perdido, por isso é importante realçar alguns termos que merecem mais destaque.

No primeiro parágrafo, por exemplo, temos o seguinte texto:
...
Aqui, seria interessante dar destaque para os termos "qualidade", "durabilidade" e "agregam valor", deixando-os em itálico, por exemplo. Para fazer isso, usamos a tag <em>:
...
É importante ressaltar aqui a parte "Não trabalho com escopo fechado", com ainda mais destaque que os termos anteriores, deixando-os em negrito. Para isso, podemos usar a tag <strong>:

<p><strong>Não trabalho com escopo fechado</strong>: o cliente é que decide quando o produto está pronto</p>
Aplique essas duas tags em seu código, neste e em outros lugares que achar relevante.


<h2>Página do blog</h2>
O site do João da Silva será composto de diversas páginas, como visto em aula. Vamos começar também a página do blog. Crie o arquivo blog.html. Nele, deve haver o seguinte texto:
...
-------------------------------------------------------------------------------------------------
<h1>Seção 02 - Começando com CSS</h1>
...
Página com algumas cores e fontes
...
-----------------------------------------------------------------------------------------
<h1>Seção 03 - Aprofundando no HTML</h1>
...
<h2>Mais semântica nos posts do blog</h2>

Na página blog.html, usamos a tag <p> para demarcar a região das tags do post:

<p>Tags: <a href="#">design responsivo</a>, <a href="#">mobile</a>, <a href="#">css</a></p>
Essa é a melhor tag para demarcar esse conteúdo? Também não demarcamos em nenhum momento cada um dos posts, isto é, nada separa um post do outro.

Podemos melhorar a semântica dos posts envolvendo cada um deles numa tag <article>, que representa um elemento que, por si só, já tem sentido completo, ou seja, cada um deles representa um bloco único de conteúdo, de artigo de post, por isso o uso da tag article.

Dentro do post, as tags são uma informação complementar diretamente relacionada ao post, são palavras chaves daquele post. Sendo assim, podemos indicar essa relação usando, no lugar da tag <p>, a tag <footer>.

Por fim, uma informação interessante em um post é sua data de publicação. Podemos marcar semanticamente datas no nosso código HTML com a tag <time> e seu atributo datetime, que especifica a data num formato mais fácil do computador processar/entender:

<time datetime="2014-07-13">13 de julho de 2014</time>
Adicione uma data de publicação para cada post usando essa tag e use as tags <article> e <footer> para melhorar a semântica dessa página, empregando através das tags corretas o significado que mais faz sentido para cada uma das partes do site.

VER OPINIÃO DO INSTRUTOR
Opinião do Instrutor

O código final da página blog.html deve ficar assim:
...
O navegador coloca uma cor padrão para os links da página. Vamos alterá-la.

No conteúdo principal, a cor deve ser #851944. Na barra lateral, deve ser idêntica à cor do texto dessa barra, que já foi configurada no exercício anterior. Aproveite para também mudar a fonte dos links para usar a fonte "Open Sans Condensed".

Dica: para não ter que repetir a cor do texto do <aside> para os links dentro dele, você pode usar o valor inherit para a cor. Esse valor faz com que a propriedade assuma o valor que foi definido em seu pai (ou ancestral, de um modo geral) em vez de assumir um outro valor. Por exemplo, se tivermos o seguinte HTML:
...
O link a, neste caso, vai obrigatoriamente pegar a cor do pai dele, o <li>, que já pega normalmente a cor do pai dele, o <ul>, que é rosa.


-------------------------------------------------------------------------------------------------------
<h1>Seção 04 - O navegador trabalhando a nosso favor</h1>
...
--------------------------------------------------------------------------------------------------------
<h1>Seção 06 - Construindo uma base sólida</h1>
...
Baixe o Normalize clicando aqui e teste-o nas suas páginas. Que diferenças você nota entre ele e o reset do Eric Meyer?
...
---------------------------------------------------------------------------------------------------
<h1>Seção 07 - Um pouquinho de posicionamento</h1>
...
--------------------------------------------------------------------------------------------------
<h1>Seção 09 - Nem tudo é o que parece</h1>
...
------------------------------------------------------------------------------------------------------
<h1>Seção 10 - Posicionamento mais a fundo</h1>
...
----------------------------------------------------------------------------------------------
<h1>Seção 11 - Elementos onde queremos</h1>

Nossa página já está quase com o layout que queremos. Falta apenas deixar a foto do João da Silva maior, cobrindo ligeiramente o resto da página.

Se aumentarmos a foto, nosso layout quebra completamente: a foto e a barra lateral descem para baixo do conteúdo principal. Isso porque estamos usando a propriedade float para posicionar esses dois elementos. Essa propriedade nos permite posicionar elementos, porém de uma forma limitada, respeitando o fluxo do documento.

Layout quebrado por causa da foto um pouco maior

Para posicionarmos elementos com controle total, precisamos da propriedade position e suas parceiras top, left, bottom e right. O comportamento dessas quatro últimas propriedades muda bastante de acordo com o valor passado à propriedade position. Vamos estudar cada um desses valores em detalhe.
...
Para conseguir esse efeito, usamos o position: relative em conjunto com top e right, inserindo espaço no topo e na direita:
...
Repare que o resto da página continua intacto! Apenas a foto é deslocada, cobrindo um pouco da barra lateral e do cabeçalho.

Agora podemos tentar aumentá-la. Colocamos uma largura de 300 pixels, por exemplo e... o resultado não é o esperado: a foto e a barra lateral descem.

Foto aumentada, fazendo o conteúdo descer

Por que isso acontece? Precisamos lembrar que a foto e a barra lateral estão posicionados com o uso da propriedade float. Quando aumentamos a foto, ela deixa de caber do lado do nosso conteúdo principal (tag <main>).

O position: relative tem vários usos interessantes. Mas nossa ideia de usar o position: relative e, depois, aumentar a imagem não vai funcionar porque nossa foto não pode influenciar os outros elementos.
...
Isso vai nos dar um resultado um tanto interessante. Primeiro, veremos que a barra lateral aparentemente desaparece. Segundo, veremos que a foto permanece ao lado do cabeçalho, só que um pouco para fora da página.
...
A foto vai para o canto esquerdo da tela e vemos que a barra lateral não desapareceu, apenas estava escondida atrás da foto.

Foto no canto superior esquerdo da página com position:absolute
...
Página como desejávamos no começo do curso

Posicionamento fixo
Podemos ir um pouco além do resultado desejado e começar a pensar na interatividade da página. O que acontece, por exemplo, quando nosso usuário faz a rolagem da página? Ou quando coloca o mouse em cima de um link? Vemos algumas dessas questões no curso avançado de HTML.

Porém, um detalhe relativamente simples de ser feito e que tem um resultado interessante é deixar o rodapé da página sempre visível, independentemente da posição da rolagem da página no navegador.

Rodapé sempre visível

Para conseguir esse resultado, usaremos mais um valor da propriedade position: o valor fixed. Ele nos permite deixar um elemento no lugar que quisermos da janela usando, também, as propriedades top, left, bottom e right para determinar a distância do elemento para esses quatro cantos da janela.
...
O resultado inicial será um pouco estranho. O rodapé deixará de ocupar toda a parte de baixo da tela.

Rodapé pequeno no canto esquerdo da tela

Podemos resolver isso determinando que a largura seja 100%. Como o rodapé tem um espaçamento interno (padding), precisamos usar o width junto com o box-sizing, para que a largura seja medida pelas bordas do rodapé, e não pelo espaço interno apenas.
...
Feito isso, teremos o resultado visual esperado, mas não conseguiremos ver o final da página. O rodapé ficará em cima do final da página.

Rodapé em cima do final da página

Para resolver esse problema, podemos colocar um espaço extra no final do conteúdo principal (tag <main>), dentro dele. Usamos, então, a propriedade padding-bottom:
...
Durante o curso, desenvolvemos duas páginas: bio.html e blog.html. Com elas, exploramos as linguagens HTML e CSS. Agora que temos esse conhecimento em mãos, podemos nos preocupar com alguns detalhes além da tecnologia e aproveitar para amadurecer nossos conhecimentos.

Organização do projeto
Um primeiro detalhe com o qual podemos nos preocupar é a organização do código no projeto. Até agora, estávamos colocando todos os arquivos juntos no mesmo diretório. Ainda não temos muitos arquivos, mas já começa a ficar meio complicado de encontrar o que queremos. Num projeto maior, isso pode ser bem complicado!

A ideia, então, é separar os arquivos em diretórios de acordo com o tipo deles. Ou seja, um diretório para imagens, um para arquivos CSS... Os arquivos HTML podem ficar no diretório principal, já que são a base do projeto.

Organização do CSS
Outra coisa que podemos notar é que nosso código CSS está ficando grande. Conforme o projeto vai crescendo, fica cada vez mais complicado manter todo o estilo num único arquivo.

Para facilitar a manutenção desse código, podemos quebrá-lo em vários arquivos de acordo com os critérios que acharmos mais interessantes. Um critério bastante comum é a página à qual aquele código se aplica. Ou seja, criamos um arquivo CSS para cada página. Nesse critério, é importante observar que parte do código é aplicado a todas as páginas (pense no cabeçalho ou na barra de navegação, por exemplo)! Assim, acabamos com a divisão: um arquivo para o estilo genérico de todas as páginas e um arquivo específico para cada página.

Consolidando o conhecimento
Para consolidar o que vimos durante o curso, deixaremos para você o desafio de criar mais uma página para o site do João da Silva: o portfólio. Essa página terá a lista dos projetos feitos por ele e quais responsabilidades ele teve em cada um. Veremos mais detalhes nos exercícios.